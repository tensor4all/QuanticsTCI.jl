@testset "MatrixCI" begin
    @testset "Matrix util" begin
        ...
        @test A ≈ QuanticsTCI.AtimesBinv(A, eye(3))
        @test A ≈ QuanticsTCI.AinvtimesB(eye(5), A)
        ...
        @test eye(3) ≈ QuanticsTCI.AtimesBinv(B, B)
        @test eye(3) ≈ QuanticsTCI.AinvtimesB(B, B)
        @test eye(5) ≈ QuanticsTCI.AtimesBinv(C, C)
        @test eye(5) ≈ QuanticsTCI.AinvtimesB(C, C)

        @test A * inv(B) ≈ QuanticsTCI.AtimesBinv(A, B)
        @test inv(C) * A ≈ QuanticsTCI.AinvtimesB(C, A)
    end

    @testset "Empty constructor" begin
        ci = QuanticsTCI.MatrixCrossInterpolation{Float64}(10, 25)
        ...
        for j in 1:25
            @test QuanticsTCI.col(ci, j) ≈ zeros(10)
            @test ci[:, j] ≈ zeros(10)
        end
    end
    ...
    @testset "Finding pivots, rank-1 case" begin
        ...
        @test ci[:, :] ≈ A
        @test QuanticsTCI.avail_rows(ci) == [1, 2]
        @test QuanticsTCI.avail_cols(ci) == [1, 2, 3]
